...
Reduces AIC from base model; adding in at end of prev model
```{r}
...
summary(mod1.1) #noticing heavy correlation w prevResponse effect?
car::vif(mod1.1) #not bad

```

### Testing previous feedback crossed effect
does it make sense to have previous feedback crossed effect or subject|feedback as a nested effect? Seems that this doesn't do much for us and is very correlated with the prevResponse variable...
...
mod3.0 <- glmer(ResponseCorrect_num ~ total_trial_z + prevRT_log + task_phase + prevFeedback*task_phase*prevResponse + (1 + task_phase |subject) + (1+ prevFeedback + task_phase| subject:prevResponse), data = data, family = binomial, control = glmerControl(optimizer = "bobyqa")) #singular
...
VarCorr(mod3) #singular task_phase
