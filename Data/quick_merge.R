...
selected <- measures_df %>% dplyr::select(RandomID, gen, PAI_Tot, PAI_AI,
                                          PAI_NR,
                                          PAI_SH,
                                          PID_neg_aff,
                                          PID_anh,
                                          ...
                                          PID_psycho, STAI_Tot,STAI_T,STAI_S,PSWQ_Tot,BFAS_Tot,BFAS_V,BFAS_W, age) %>% rename(subject = RandomID) %>% mutate(subject = as.character(subject)) 
 
df <- left_join(selected, within_effects, by = "subject")
