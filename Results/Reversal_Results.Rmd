...
**Subject**. We are intersted in within-subject varaition, for sure. 
...
```{r}
minfo[["mod8"]] <- c(fixed="tot_trial_z, PrevRT, phase*prevFeed, prevFeed*prevResp", l2="Intercept + PrevRESP/Subject")
mod8 <- glmer(ResponseCorrect_num ~ total_trial_z + prevRT_log + task_phase*prevResponse + prevFeedback*task_phase + prevFeedback*prevResponse + (1 + prevResponse + prevFeedback + task_phase|subject), data = data, family = binomial, control = glmerControl(optimizer = "bobyqa"))
AIC(mod6.4, mod8)
```
...
```{r}
minfo[["mod8"]] <- c(fixed="tot_trial_z, PrevRT, phase*prevFeed, prevFeed*prevResp", l2="Intercept + PrevRESP/Subject")
mod8 <- glmer(ResponseCorrect_num ~ total_trial_z + prevRT_log + task_phase*prevResponse + prevFeedback*task_phase + prevFeedback*prevResponse + (1 + prevResponse|subject), data = data, family = binomial, control = glmerControl(optimizer = "bobyqa"))

minfo[["mod8.1"]] <- c(fixed="tot_trial_z, PrevRT, phase*prevFeed, prevFeed*prevResp", l2="Intercept + PrevRESP/Subject")
mod8 <- glmer(ResponseCorrect_num ~ total_trial_z + prevRT_log + task_phase*prevResponse + prevFeedback*task_phase + prevFeedback*prevResponse + (1 |subject) + (1 |prevResponse), data = data, family = binomial, control = glmerControl(optimizer = "bobyqa"))

AIC(mod6.4, mod8, mod8.1)
```

#### Previous Feedback
Not sure quite how bad this is...
```{r}
minfo[["mod8.2"]] <- c(fixed="tot_trial_z, PrevRT, phase*prevFeed, prevFeed*prevResp", l2="Intercept + PrevRESP/Subject")
mod8.2 <- glmer(ResponseCorrect_num ~ total_trial_z + prevRT_log + task_phase*prevResponse + prevFeedback*task_phase + prevFeedback*prevResponse + (1 + prevFeedback |subject) + (1 |prevResponse), data = data, family = binomial, control = glmerControl(optimizer = "bobyqa")) #singularity here

minfo[["mod8.3"]] <- c(fixed="tot_trial_z, PrevRT, phase*prevFeed, prevFeed*prevResp", l2="Intercept + PrevRESP/Subject")
mod8.3 <- glmer(ResponseCorrect_num ~ total_trial_z + prevRT_log + task_phase*prevResponse + prevFeedback*task_phase + prevFeedback*prevResponse + (1 |subject) + (1 |prevResponse/prevFeedback), data = data, family = binomial, control = glmerControl(optimizer = "bobyqa"))
AIC( mod8, mod8.1)
```
